...
class BinBG_Model
{
    ...
    public static function doImport($data) {
        ...
        foreach ($data as $key => $value) {
            ...
            $sql = 'select id from bin_bg where bin =' . $bin . ' and pno =\''.$value['1'].'\'';
            $bin_bg = DB::one($sql);
            if(empty($bin_bg)){
        ...
        if(!empty($data))
        {
            $sql = 'insert into `bin_bg` (`'.implode('`,`',array_keys($title)).'`,`imported_at`) values '.implode(',',$data);
            if(DB::query($sql)){
    ...
    public static function load($pno, $bid) {
        ...
        $sql = "select `current`, `index`, `ctemp`, `bright`, `voltage`, `ccode` from `bin_bg` where `bin` = '{$bid}' and `pno` like '" . $pno ."%'";
